...
struct DiLepton
{
  ...
  DiLepton(TLorentzVector& L1_, TLorentzVector& L2_)
    : L1(L1_), L2(L2_), Z(L1_+L2_), used(false) {}
  ~DiLepton() {}
  TLorentzVector L1;
  TLorentzVector L2;
  TLorentzVector Z;
...
void monoHiggs::analysis4mu(string inputFile,
  ...
  //TH1F* InvMass = new TH1F("InvMass","InvMass", 100, 50. , 150.);
  ...
  //TH1F* fourLmass = new TH1F("fourLmass","fourLmass",300,0.,150.);
  TH1F* Hmass = new TH1F("Hmass","Hmass",200,50.,150.);
  TH1F* HmassLarge = new TH1F("HmassLarge","HmassLarge",600,0.,300.);
  //TH1F* HmassAll = new TH1F("HmassAll","HmassAll",300,0.,300.);
  //TH1F* HmassAllMela = new TH1F("HmassAllMela","HmassAllMela",300,0.,300.);
  //TF1*  Z1fit = new TF1("Z1fit","gaus",83,100);
  //TF1*  Hfit = new TF1("Hfit","gaus",100,140);
  //TF1*  fitFcnZ = new TF1("fitFcnZ",fitFunction,60.,110.,7);
  ...
  std::cout <<"\tanalyzing "<<numberOfEntries<<" events" << std::endl;
   ...
   for(Int_t entry = 0; entry < numberOfEntries; entry++){
    ...
    // find Z candidates from potential Z candidates
    int indexZ1 = 0;
    double error = 1e10;
    double largestPT = 0;
    ...
    for(size_t k = 0; k < dilepton.size(); k++){
      ...
      if(fabs(d.Z.M() - ZMASS) < error){
	indexZ1 = k;
	error = fabs(d.Z.M() - ZMASS);
      }
    }
    
    // flag this dilepton as "used"
    dilepton[indexZ1].used = true; // NB. flag as used
    Z1 = dilepton[indexZ1].Z;
    L1 = dilepton[indexZ1].L1;
    L2 = dilepton[indexZ1].L2;    
    std::cout << "\tevent[" << entry << "] Z1mass =" << Z1.M() << std::endl;
    ...
    // now look for another Z candidate
    for(size_t k = 0; k < dilepton.size(); k++){
      ...
      double dR = nic::deltaR(d.Z.Eta(), d.Z.Phi(),
			      Z1.Eta(), Z1.Phi());
      if ( ! (dR > 0.02) )   continue;
    ...
    std::cout << "\tevent[" << entry << "] Z2mass =" << Z2.M() << std::endl;   
    // ----------------------------------------------------------
    // sort lepton PTs
    // ----------------------------------------------------------
   ...
   if ( masses ){
     ...
     TCanvas *cuts = new TCanvas();
     cuts->cd();
     TH1F *h_nEvent1 = new TH1F("h_nEvent1","Delphes Eff vs cut",12,-1,11);
     h_nEvent1->Sumw2();
